...
// Mapping des corrections secteur -> sous-secteur
const industrySubindustryMapping = {
  ...
  'Technology, Information and Media': [
    ...
    'IT Services and IT Consulting'  // Correction: déplacer de Professional Services
  ],
...
async function fixIndustryDependencies() {
  const client = await pool.connect();
  
  try {
    console.log('🔧 Début de la correction des dépendances secteur/sous-secteur...');
    ...
    console.log('📊 Avant correction (top 10):');
    ...
    // Correction spéciale pour IT Services and IT Consulting
    const itServicesResult = await client.query(`
      UPDATE contacts 
      SET current_company_industry = 'Technology, Information and Media'
      WHERE current_company_subindustry = 'IT Services and IT Consulting'
        AND current_company_industry = 'Professional Services'
      RETURNING id
    `);
    
    if (itServicesResult.rowCount > 0) {
      console.log(`  ✅ ${itServicesResult.rowCount} contacts IT Services déplacés vers Technology, Information and Media`);
      totalUpdated += itServicesResult.rowCount;
    }
