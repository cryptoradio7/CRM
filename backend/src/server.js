...
app.get('/api/contacts', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
      ...
      WHERE 
        ${hasAccents ?
            'LOWER(c.full_name) LIKE $1 OR LOWER(c.full_name) LIKE $2 OR LOWER(c.current_company_name) LIKE $1 OR LOWER(c.current_company_name) LIKE $2' :
            'LOWER(c.full_name) LIKE $1 OR LOWER(c.current_company_name) LIKE $1'}
...
app.get('/api/companies', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
      ...
      `, [searchPattern]);
        ...
        const countResult = yield pool.query('SELECT COUNT(DISTINCT c.company_name) FROM companies c');
        const totalCount = parseInt(countResult.rows[0].count);
        const result = yield pool.query(`
      SELECT 
        c.*,
...
app.get('/api/companies/:id', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const companyResult = yield pool.query(`
    ...
    `, [id]);
        ...
        const countResult = yield pool.query(countQuery, [id]);
      ...
      `
        ...
        LIMIT $2 OFFSET $3
      `;
        const employeesResult = yield pool.query(employeesQuery, [id, parseInt(limit), offset]);
        res.json(Object.assign(Object.assign({}, companyResult.rows[0]), { employees: employeesResult.rows, pagination: {
                page: parseInt(page),
                limit: parseInt(limit),
...
app.get('/api/companies/:id/employees', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const { id } = req.params;
    ...
    `;
        const employeesResult = yield pool.query(employeesQuery, [id, parseInt(limit), offset]);
        // Compter le total
        const countResult = yield pool.query('SELECT COUNT(DISTINCT e.id) as total FROM experiences e WHERE e.company_id = $1', [id]);
        const totalEmployees = parseInt(countResult.rows[0].total);
        res.json({
...
app.post('/api/contacts/import', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        for (const contact of dataToImport) {
            try {
                ...
                let contactId;
                const existingContact = yield pool.query('SELECT id FROM contacts WHERE lead_id = $1', [contact.lead_id]);
          ...
          `, [
                ...
                yield pool.query('DELETE FROM experiences WHERE contact_id = $1', [contactId]);
                yield pool.query('DELETE FROM contact_languages WHERE contact_id = $1', [contactId]);
                yield pool.query('DELETE FROM contact_skills WHERE contact_id = $1', [contactId]);
                yield pool.query('DELETE FROM contact_interests WHERE contact_id = $1', [contactId]);
                yield pool.query('DELETE FROM contact_education WHERE contact_id = $1', [contactId]);
